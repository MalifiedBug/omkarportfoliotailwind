const features = [
  ...
  },
  {
    ...
    description:
      "S.E.S Chandak College of Art's, Commerce and Science, Solapur.",
    marks: "72.33%",
  ...
  {
    name: "B.E.-Electronics & Telecommunications",
    description: "Savitribai Phule Pune University, Pune.",
    marks: "Higher Second Class",
    src: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQgVd_JlfAz_A8dwC-uVE2OXb_-TF6FEhTKel34LRwiZrbhsMmatZv2W1FCsH5HshzZMQY&usqp=CAU",
    passing: "2020",
  },
  {
      ...
      <div>
        ...
        <dl className="mt-16 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 sm:gap-y-16 lg:gap-x-8">
          {features.map((feature) => (
            <div
              key={feature.name}
              className="border-t text-2xl flex flex-col border-gray-200 pt-4"
            >
              <dt className="font-medium grid grid-cols-5 gap-3 text-gray-900">
                <img
                  src={feature.src}
                  alt={feature.name}
                  className="rounded-full  object-cover sm:h-20 sm:w-20 lg:h-32 lg:w-32 bg-gray-100"
                />
                <b className="col-span-3 flex items-center justify-center">
                  {feature.name}
                </b>
              </dt>
              <dd className="mt-2 text-xl text-gray-500">{feature.passing}</dd>
              <dd className="mt-2 text-xl text-gray-500">
                {feature.description}
              </dd>
              <dd className="mt-2 text-xl text-gray-500">{feature.marks}</dd>
            </div>
          ))}
        </dl>
      </div>
