        ...
        <div>
          ...
          <dl className="mt-16 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 sm:gap-y-16 lg:gap-x-8">
            {features.map((feature) => (
              <div key={feature.name} className="border-t text-2xl flex flex-col border-gray-200 pt-4">
                
                <dt className="font-medium grid grid-cols-5 gap-3 text-gray-900"><img
                    src={feature.src}
                    alt={feature.name}
                    className="rounded-full  object-cover sm:h-20 sm:w-20 lg:h-40 lg:w-40 bg-gray-100"
                  /><b className="col-span-3 flex items-center justify-center">{feature.name}</b></dt>
